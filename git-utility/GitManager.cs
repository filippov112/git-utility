...
namespace GitUtility
{
    public class GitManager
    {
        ...
        private async Task StartCommand()
        {
            ...
            Console.WriteLine($"Создана рабочая ветка '{branchName}'");
        }
        ...
        private async Task FixCommand(string description = "")
        {
            ...
            if (activeCommitId.HasValue)
            {
                ...
                if (childrenCount > 0)
                {
                    ...
                    await RunGitCommand($"checkout -b {branchName}");
                    Console.WriteLine($"Создана ветка {branchName}. Выполнено переключение!");
                }
            }
        ...
        private async Task ShowCommand(int? specificCommit = null)
        {
            ...
            var commits = await _dbManager.GetAllCommits(_repositoryId);
        ...
        private async Task SaveCommand()
        {
            ...
            if (nextNumber > 2)
            {
                ...
                Console.WriteLine("Pull Request создан... (Не забудьте его принять)");
            }
            else
            {
                Console.WriteLine("Изменений в сессии не обнаружено...");
            }
            
            // Возврат на основную ветку
            await RunGitCommand("checkout main");
            Console.WriteLine("Переключение на ветку main...");

            // Удаление локальной рабочей ветки
            await RunGitCommand($"branch -D {currentBranch}");
            Console.WriteLine($"Ветка {currentBranch} удалена...");

            // Очистка дерева коммитов
            await _dbManager.ClearCommitTree(_repositoryId);
            Console.WriteLine("Дерево очищено, сессия закрыта!");
        }
